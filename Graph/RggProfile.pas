...
type
  ...
  protected
    { not used but not yet deleted }
    procedure InternalDrawProfile1(Canvas: TCanvas);
    procedure InternalDrawProfile2(Canvas: TCanvas);
    procedure InternalDrawProfile4(Canvas: TCanvas);
    procedure InternalDrawProfile5(Canvas: TCanvas);
  private
...
procedure TMastProfile.InternalDrawProfile1(Canvas: TCanvas);
  ...
  begin
    ...
    begin
      ...
      x1 := x1 / ControllerZoom1;
      y1 := y1 / ControllerZoom1;
      x2 := x2 / ControllerZoom1;
      y2 := y2 / ControllerZoom1;
  ...
  begin
    ...
    begin
      xm := xm + OffsetX;
      ym := ym + OffsetY;
      xm := xm / ControllerZoom1;
      ym := ym / ControllerZoom1;
      Radius := Radius / ControllerZoom1;
      a1 := 90 - phi1;
      a2 := phi1-phi2;
...
procedure TMastProfile.InternalDrawProfile2(Canvas: TCanvas);

  procedure MetaLINE(x1, y1, x2, y2: single);
  begin
    if Lage = Quer then
    begin
      x1 := x1 * ControllerZoom2 + OffsetX;
      y1 := y1 * ControllerZoom2 + OffsetY;
      x2 := x2 * ControllerZoom2 + OffsetX;
      y2 := y2 * ControllerZoom2 + OffsetY;
      Canvas.DrawLine(PointF(y1, x1), PointF(y2, x2), 1.0);
    end
    else if Lage = Hoch then
    begin
      x1 := x1 * SalingZoom2 + OffsetX;
      y1 := y1 * SalingZoom2 + OffsetY;
      x2 := x2 * SalingZoom2 + OffsetX;
      y2 := y2 * SalingZoom2 + OffsetY;
      Canvas.DrawLine(PointF(x1, y1), PointF(x2, y2), 1.0);
    end;
  end;
  ...
  begin
    if Lage = Quer then
    begin
      xm := xm * ControllerZoom2 + OffsetX;
      ym := ym * ControllerZoom2 + OffsetY;
      Radius := Radius * ControllerZoom2;
      a1 := 90 - phi1;
      a2 := phi1-phi2;
      Canvas.DrawArc(PointF(ym, xm), PointF(Radius, Radius), a1, a2, 1.0);
    end
    ...
    begin
      xm := xm * SalingZoom2 + OffsetX;
      ym := ym * SalingZoom2 + OffsetY;
      Radius := Radius * SalingZoom2;
      a1 := phi1;
      a2 := phi2-phi1;
      Canvas.DrawArc(PointF(xm, ym), PointF(Radius, Radius), a1, a2, 1.0 );
    end;
...
begin
{ MetaLINE(    x1,   y1,    x2,     y2); }
  MetaLINE(   49.0,  99.7,  104.4,    52.0);
  MetaLINE(  285.0, 407.0,  285.0,   435.0);
  MetaLINE(   35.0, 130.0,    0.0,   130.0);
  MetaLINE(  114.8,  60.9,   35.0,   130.0);
  MetaLINE(   20.0,   0.0,   50.0,     0.0);
  MetaLINE(  260.0, 318.5,  260.0,   490.0);

  MetaLINE(  -48.5, 100.2, -104.4,    52.0);
  MetaLINE(    0.0, 130.0,  -35.0,   130.0);
  MetaLINE(  -35.0, 130.0, -115.5,    60.4);
  MetaLINE( -285.0, 435.0, -285.0,   407.0);
  MetaLINE(  -50.0,   0.0,  -20.0,     0.0);
  MetaLINE( -260.0, 490.0, -260.0,   318.5);
{ MetaLINE(     0, 7200,     0,      0); }

{ MetaARC(     xm,   ym,     r,    phi1,    phi2); }
  MetaARC(  -185.0, 407.0,  458.0,  -59.83,  -50.83);
  MetaARC(  -185.0, 407.0,  458.0,  -49.09,  -16.70);
  MetaARC(  -185.0, 407.0,  470.0,  -60.00,    0.00);
  MetaARC(     0.0,  43.0,   75.0,   49.15,   90.00);
  MetaARC(     0.0, 435.0,  285.0,    0.00,   90.00);
  MetaARC(     0.0, 435.0,  273.0,   26.58,   90.00);
  MetaARC(    35.0,  00.0,   15.0,   47.46,  180.00);
  MetaARC(   110.0, 318.5,  150.0,  -16.71,    0.00);
  MetaARC(   110.0, 490.0,  150.0,    0.00,   26.58);

{ MetaARC(     xm,   ym,     r,    phi1,    phi2); }
  MetaARC(  -110.0, 490.0,  150.0,  153.42, 180.00);
  MetaARC(  -110.0, 318.5,  150.0, -180.00, -163.29);
  MetaARC(   -35.0,   0.0,   15.0,    0.00,  132.53);
  MetaARC(     0.0,  43.0,   75.0,   90.00,  130.85);
  MetaARC(     0.0, 435.0,  285.0,   90.00, 180.00);
  MetaARC(     0.0, 435.0,  273.0,   90.00,  153.42);
  MetaARC(   185.0, 407.0,  458.0, -129.17, -120.16);
  MetaARC(   185.0, 407.0,  458.0, -163.29, -130.91);
  MetaARC(   185.0, 407.0,  470.0, -180.00, -120.00);
end;
...
procedure TMastProfile.InternalDrawProfile4(Canvas: TCanvas);
...
procedure TMastProfile.InternalDrawProfile5(Canvas: TCanvas);
